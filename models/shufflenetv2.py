...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        x1, x2 = self.split(x)
        out = self.act1(self.bn1(self.conv1(x2)))
...
class DownBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = self.act2(self.bn2(self.conv2(out1)))
        # right
        out2 = self.act3(self.bn3(self.conv3(x)))
        out2 = self.bn4(self.conv4(out2))
        out2 = self.act5(self.bn5(self.conv5(out2)))
        # concat
...
class ShuffleNetV2(nn.Module):
    def __init__(self, net_size=1, num_classes=10):
        ...
        out_channels = CFG[net_size]['out_channels']
        num_blocks = CFG[net_size]['num_blocks']
...
def shufflenetv2 (**kwags):
    return ShuffleNetV2(**kwags)